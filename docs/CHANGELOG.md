...
- Added **Metric Extractor** capability (productivity category)
  ...
  - Full usage logging for self-improvement loop
  - Wired into capability registry + API route + intent classifier signals
  - Complements deadline (when) and priority (where attention goes) with metric (how we know it worked)
- Quantum Core version bump to 2.2.0
- Direct progress toward "Work Less. Live More." — work gets a finish line so effort is not spent without a definition of done
...
## Earlier
- See prior versions for extractors v0.1–v0.9 and agent/admin foundation.
